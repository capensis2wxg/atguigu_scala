package com.capensis.chapter18generic.contextbounds

object ContextBoundsDemo {
	//这里我定义一个隐式值  Ordering[Person]类型
	implicit val personComparetor: Ordering[Person4] = new Ordering[Person4] {
...
class CompareComm4[T: Ordering](obj1: T, obj2: T)(implicit comparetor: Ordering[T]) {
	def greater: T = if (comparetor.compare(obj1, obj2) > 0) obj1 else obj2
}
...
class CompareComm5[T: Ordering](o1: T, o2: T) {
	def greater: T = {
		def f1(implicit cmptor: Ordering[T]): Int = cmptor.compare(o1, o2) //返回一个数字
...
class CompareComm6[T: Ordering](o1: T, o2: T) {
	def greater: T = {
		...
		val comparetor: Ordering[T] = implicitly[Ordering[T]]
